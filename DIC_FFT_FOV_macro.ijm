...
function processFile(input, output, file) {
    ...
    for (s = 0; s < 3; s++) {
		...
		run("Bio-Formats Importer", "open=[" + path + "] color_mode=Default view=Hyperstack stack_order=XYCZT series_" + s + " specify_series");
        ...
        saveAs("Tiff", output2 + File.separator + "60C_" + title2 + "_series" + (s+1) + ".tif");
